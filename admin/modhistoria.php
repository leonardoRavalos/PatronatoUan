...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['guardar'])) {
    $idinteres = $_POST['idfoto_historia'];
    ...
    $sql = "UPDATE fotos_historia SET archivo = '$nuevaImagen'WHERE idfoto_historia = $idinteres";
...
if (isset($_GET['id'])) {
    ...
    $query = "SELECT idfoto_historia,  archivo FROM fotos_historia WHERE idfoto_historia = $idinteres";
    $result = mysqli_query($conn, $query);

    if ($row = mysqli_fetch_assoc($result)) {
        $imagenActual = $row['archivo'];
    } else {
...
<body>
    <h1>Editar imagen historia</h1>
    <form method="POST" enctype="multipart/form-data">
        <input type="hidden" name="idfoto_historia" value="<?php echo $idinteres; ?>">
