...
<head>
    ...
    <title>Agregar imagen historia</title>
</head>
            ...
            <header>
                ...
                <nav class="menu">
                    <ul>
                        ...
                        <li><a href="intereses.php">Interés</a></li>
                    ...
                    <td>
                        <table border="0" width="100%" cellspacing="0" cellpadding="0">
                            ...
                            <tr>
                                ...
                                <td width="51%"><font face="Arial">&nbsp;&nbsp;</font><font size="3" face="Arial"><input
                                            type="file" name="imagen" id="imagen"></font></td>
                            </tr>
                            <tr>
                                <td width="11%">&nbsp;</td>
            ...
            if ($_FILES['imagen']['name']) {
                ...
                if (in_array($imagen_extension, $extensiones_validas)) {
                    ...
                    if (move_uploaded_file($imagen_temp, $imagen_ruta)) {
                        // Obtén el valor de idpantalla del formulario
                        $idpantalla = $_POST['imagen'];

                        // Inserta los datos en la base de datos usando una sentencia preparada
                        $sql = "INSERT INTO pantalla_principal (idpantalla, imagen) VALUES (?, ?)";
                        if ($stmt = $conn->prepare($sql)) {
                            $stmt->bind_param("ss", $idpantalla, $imagen_nombre);
                            if ($stmt->execute()) {
                                // Éxito, la imagen se ha agregado
                                echo "Imagen agregada correctamente.";
                            } else {
                                echo "Error al agregar la imagen: " . $stmt->error;
                            }
                            $stmt->close();
                        } else {
