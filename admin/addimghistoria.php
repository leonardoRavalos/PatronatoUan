    ...
    if ($_FILES['imagen']['name']) {
        ...
        if (in_array($imagen_extension, $extensiones_validas)) {
            ...
            if (move_uploaded_file($imagen_temp, $imagen_ruta)) {
                ...
                $sql = "INSERT INTO fotos_historia (historia, archivo) VALUES (?, ?)";
                if ($stmt = $conn->prepare($sql)) {
                    $historia = 1; // Reemplaza con el ID de historia correspondiente
                    $stmt->bind_param("ss", $historia, $imagen_nombre);
                    if ($stmt->execute()) {
